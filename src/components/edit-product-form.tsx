...
import { Plus, Minus, Loader2, CheckCircle, ImagePlus, X, Trash2 } from "lucide-react";
...
const NUTRIMENT_FIELDS: { key: NutrimentKey; label: string; unit: string; step?: string }[] = [
  ...
  { key: "salt_100g", label: "Salt", unit: "g", step: "0.01" },
];

function initNutriments(nm?: OFFProduct["nutriments"]): Partial<Record<NutrimentKey, string>> {
  if (!nm) return {};
  const out: Partial<Record<NutrimentKey, string>> = {};
  for (const key of NUTRIMENT_FIELDS.map((f) => f.key)) {
    const v = nm[key];
...
export function EditProductForm({ product }: { product: OFFProduct }) {
  ...
  // Image — start with existing data url if present
  const [imagePreview, setImagePreview] = useState<string | null>(product.image_data_url ?? null);
  ...
  const [name, setName] = useState(product.product_name_en || product.product_name || "");
  const [brand, setBrand] = useState(product.brands ?? "");
  const [categories, setCategories] = useState(product.categories ?? "");
  const [servingSize, setServingSize] = useState(product.serving_size ?? "");
  const [ingredients, setIngredients] = useState(product.ingredients_text_en || product.ingredients_text || "");
  const [nutriScore, setNutriScore] = useState(product.nutriscore_grade && !["unknown","not-applicable"].includes(product.nutriscore_grade) ? product.nutriscore_grade : "");
  const [novaGroup, setNovaGroup] = useState(product.nova_group ? String(product.nova_group) : "");
  const [nutriments, setNutriments] = useState<Partial<Record<NutrimentKey, string>>>(initNutriments(product.nutriments));
  const [selectedAllergens, setSelectedAllergens] = useState<string[]>(
    (product.allergens_tags ?? []).map((t) => t.replace("en:", ""))
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => router.push(`/product/${product.code}`), 1200);
    } catch {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/product/delete?code=${encodeURIComponent(product.code)}`, { method: "DELETE" });
      router.push("/search");
    } catch {
      setDeleting(false);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <CardContent>
          {imagePreview ? (
            <div className="relative w-fit">
              ...
              <img src={imagePreview} alt="Preview" className="h-40 w-40 rounded-xl object-contain bg-muted p-2" />
              <button
                type="button"
                onClick={clearImage}
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={product.code} disabled className="rounded-xl opacity-50 cursor-not-allowed" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={servingSize} onChange={(e) => setServingSize(e.target.value)} className="rounded-xl" />
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <div className="flex gap-1.5">
              {["a", "b", "c", "d", "e"].map((g) => (
                <button key={g} type="button" onClick={() => setNutriScore(nutriScore === g ? "" : g)}
                  className={`flex-1 rounded-lg py-2 text-sm font-black uppercase transition-colors ${
                    nutriScore === g
                      ? g === "a" ? "bg-green-600 text-white" : g === "b" ? "bg-lime-500 text-white"
                        : g === "c" ? "bg-yellow-400 text-black" : g === "d" ? "bg-orange-500 text-white"
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-1.5">
              {[1, 2, 3, 4].map((n) => (
                <button key={n} type="button" onClick={() => setNovaGroup(novaGroup === String(n) ? "" : String(n))}
      ...
      <Card className="rounded-2xl border-border">
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-3">
            {NUTRIMENT_FIELDS.map(({ key, label, unit, step }) => (
              <div key={key} className="space-y-1.5">
                <label className="text-sm font-medium">{label}</label>
                <div className="relative">
                  <Input
                    type="number" min="0" step={step ?? "0.1"}
                    value={nutriments[key] ?? ""}
                    onChange={(e) => setNutriment(key, e.target.value)}
                    placeholder="0" className="rounded-xl pr-10"
                  />
      ...
      <Button type="submit" disabled={loading || !name.trim()} className="w-full rounded-xl h-11 gap-2">
        {loading
      ...
      <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-5 space-y-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Permanently remove this product from the database. This cannot be undone.</p>
        </div>
        <button
          type="button"
        ...
        >
          ...
          {deleting ? "Deleting…" : confirmDelete ? "Confirm — delete this product" : "Delete product"}
        </button>
